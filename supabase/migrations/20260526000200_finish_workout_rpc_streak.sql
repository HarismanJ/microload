-- Extend finish_workout_session_atomic with streak anchor params.
--
-- PostgreSQL function overloading: CREATE OR REPLACE with a different parameter
-- list creates a NEW overload rather than replacing the old one. We must DROP
-- the old 11-param signature first so PostgREST sees only one function.

begin;

DROP FUNCTION IF EXISTS public.finish_workout_session_atomic(
  uuid,
...
create or replace function public.finish_workout_session_atomic(
  ...
  -- Streak anchor columns (optional — COALESCE means omitting them is a no-op):
  p_streak_start_date           date default null,
  p_streak_last_workout_at      timestamp with time zone default null
)
returns void
language plpgsql
volatile
...
revoke all on function public.finish_workout_session_atomic(
  uuid, timestamp with time zone, jsonb, integer, uuid, text,
  integer, double precision, jsonb, jsonb, jsonb, date, timestamp with time zone
) from public;
revoke all on function public.finish_workout_session_atomic(
  uuid, timestamp with time zone, jsonb, integer, uuid, text,
  integer, double precision, jsonb, jsonb, jsonb, date, timestamp with time zone
