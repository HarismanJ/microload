...
export const SCHEDULED_DELOAD_SET_MULTIPLIER = 0.5
export const SCHEDULED_DELOAD_LOAD_MULTIPLIER = 0.875
export const SCHEDULED_DELOAD_CARDIO_MULTIPLIER = 0.6
export const SCHEDULED_DELOAD_TARGET_RIR = 4
...
export function applyScheduledDeloadToSuggestion(suggestion, exercise) {
  if (!suggestion || !exercise?.planDeloadWeek) return suggestion

  const suggestedWeightKg = Number.isFinite(Number(suggestion.suggestedWeightKg)) && Number(suggestion.suggestedWeightKg) > 0
    ? Math.round(Number(suggestion.suggestedWeightKg) * SCHEDULED_DELOAD_LOAD_MULTIPLIER * 10) / 10
