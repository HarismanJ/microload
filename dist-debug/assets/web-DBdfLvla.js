import { i as WebPlugin, n as CapacitorException, r as ExceptionCode } from "./dist-B65an-qx.js";
import { n as BarcodeValueType, r as LensFacing } from "./definitions-DSvdkVWR.js";
//#region node_modules/@capacitor-mlkit/barcode-scanning/dist/esm/web.js
var BarcodeScannerWeb = class extends WebPlugin {
	...
	async startScan(options) {
		if (!this._isSupported) throw this.createUnimplementedException();
		if (!(options === null || options === void 0 ? void 0 : options.videoElement)) throw new Error(this.errorVideoElementMissing);
		this.videoElement = options.videoElement;
		this.stream = await navigator.mediaDevices.getUserMedia({
			video: { facingMode: { ideal: (options === null || options === void 0 ? void 0 : options.lensFacing) === LensFacing.Front ? "user" : "environment" } },
			audio: false
		});
		...
		this.intervalId = window.setInterval(async () => {
			...
			else this.handleScannedBarcodes(barcodes);
		}, 500);
	}
	async stopScan() {
		if (!this._isSupported) throw this.createUnimplementedException();
	...
	handleScannedBarcodes(barcodes) {
		const result = { barcodes: barcodes.map((barcode) => ({
			cornerPoints: [
...
//# sourceMappingURL=web-DBdfLvla.js.map